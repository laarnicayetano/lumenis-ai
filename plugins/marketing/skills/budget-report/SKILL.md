...
2. **AOP budget sheets (Smartsheet, MCP)** — the AOP now lives on Smartsheet, not in a local Excel file. **Current location is the `Test` workspace, not `2026 - PR Entry Tracking`** — these were built there because the account only has Editor (not Admin) access to `2026 - PR Entry Tracking`, which blocks sheet creation. If someone moves them (or grants Admin access there and they get migrated), re-resolve the IDs via `search`/`browse_workspace` and update this file rather than assuming these IDs still work.
   ...
   - Each `... Budget Detail` sheet is line-item grain (one row per event/vendor, same idea as the PR trackers): `Event` (primary column), `Category` (picklist — division's own AOP categories, e.g. Workshops/Webinars/Industry Initiatives/Tradeshows/Digital/Outsourced Services — AES has Digital, VIS/Hospital don't), `Status` (Active/Cancelled), `Q1`-`Q4`, `FY26` (formula column — `=Q1+Q2+Q3+Q4` — always trust this over hand-summing), `KOL Fees`, `# of KOLs`, `Shared Event Group` (tags the AES row and VIS row of a split cross-division event, e.g. an Accelerate), `Reported Actual (AOP-era)` (a stale manually-entered actual from the old workbook, historical reference only — never use this as a stand-in for Step 3's reconciled actual), and `Notes`.

## Scripts (use these — don't re-derive this logic inline)

This skill's folder has three scripts (stdlib-only Python, no API-credential dependency, and no MCP access — the Smartsheet queries in Steps 1-2 and 4 still have to happen through the live MCP tools first):

- **`scripts/reconcile_pr_data.py <saved get_sheet_summary JSON> [--oldest-n 10]`** — takes the JSON that Smartsheet's `get_sheet_summary` returns (or saves to disk when the result is too large to inline, or `merge_sheet_pulls.py`'s output when the pull had to be split — see below) and applies Step 2's cleaning + Step 3's reconciliation + category/quarter aggregation. **The query that produces this input must include `PR#`, `PO#`, and `Date into SAP`**, not just `PO SAP Balance` — `PO#` lets the script correctly separate rows Open POPR simply hasn't matched (usually because the PO is fully closed out — see Step 3) from rows that don't have a PO# yet at all, and `PR#`/`Date into SAP` feed the two PR lists in Steps 7-8. Outputs reconciled totals, a category breakdown, a quarter breakdown, both counts (`rows_with_no_po_yet`, `rows_with_po_unmatched_in_open_popr`), any negative-reconciled-spend rows (a data-quality flag, not something the script can resolve on its own), the full `rows_without_po` list (Step 7), and the `oldest_open_prs` list (Step 8, 10 by default — pass `--oldest-n` for a longer list).
- **`scripts/aop_budget_aggregate.py <saved get_sheet_summary JSON>`** — takes a Budget Detail sheet pull (`Event, Category, Status, FY26` at minimum) and outputs per-category `FY26` totals for `Status = Active` rows, plus the excluded (Cancelled) rows and their count. **Use this for Step 4 instead of `get_sheet_aggregates`** — that tool's `group_by` only returns row *counts* per value and `summary_stats` only returns overall (not per-category) numeric stats, so neither one can actually produce "sum of `FY26` by Category," and reaching for it first just burns a round trip before falling back to this anyway. The Budget Detail sheets are small (well under 100 rows per division as of FY26), so pulling the full sheet and summing locally is both correct and cheap.
- **`scripts/merge_sheet_pulls.py <out.json> <in1.json> <in2.json> [...]`** — concatenates the `rows` arrays from multiple `get_sheet_summary` pulls into one file shaped for the two scripts above. Needed whenever a query has to be split across several calls to avoid sampling (see the VIS note in Step 2) instead of hand-copying rows between files, which is slow and easy to transcribe wrong. No dedup is performed, so keep the split filters mutually exclusive and collectively exhaustive.

Run one `reconcile_pr_data.py` call and one `aop_budget_aggregate.py` call per division. Feed both outputs into Steps 4-6 below rather than re-deriving the cleaning/reconciliation/aggregation math from the prose description each time.
...
Pull each division's rows with `get_sheet_summary`, filtered to that division's Marketing cost center (Step 1), selecting at minimum: `PR#, Header Text, Quarter, Category, Status, Valuation Price, PO#, PO SAP Balance, Date into SAP`. (`PR#` and `Date into SAP` are needed for Steps 7-8's PR lists, not just the reconciliation math.)

**Sampling note — split VIS by quarter up front; don't wait to discover it's sampled.** A `get_sheet_summary` call against a PR Tracking sheet can get silently sampled once the row count is high and this many columns are selected. VIS PR Tracking's Marketing-cost-center row count (228 as of FY26) already sits past this point. Rather than pulling VIS in one call, checking `metadata.isSampled`, and re-querying when it's true (three round trips instead of one), query it in three calls from the start and merge them with `scripts/merge_sheet_pulls.py` (see "Scripts" above): `Quarter IN ["Q126","Q226"]`, `Quarter IN ["Q326","Q426","Q127"]`, and `Quarter IS_BLANK` (blank-Quarter rows exist in this sheet and won't match either `IN` list — omitting this third call silently drops them). AES's row count (194 as of FY26) is currently under the sampling threshold and can be pulled in one call, but check `metadata.isSampled`/`rowsActual` vs `rowsInFilter` on it too — it'll cross the same line as more PRs get entered through the year, at which point apply the same three-way split to it. Re-verify both divisions' row counts each run rather than assuming this year's split (or lack of one) still holds.

Save the result (or the merged file) and run it through `scripts/reconcile_pr_data.py` (see "Scripts" above) — it applies cleaning steps 1-2 below itself (cancelled-status filter, wrong-FY-quarter filter including the blank-Quarter/Header-Text check) and reports exactly what it excluded and why in its `rows_excluded` output. **Do not skip feeding data through this script and re-doing the cleaning by hand or in prose** — in the source session, skipping equivalent steps overstated one division's spend by ~$570K and understated the other's headroom by a comparable amount. The two things the script does NOT do for you:
...
Pull the category-level AOP budget by calling `get_sheet_summary` against the division's `... Budget Detail` sheet for `Event, Category, Status, FY26`, then run it through `scripts/aop_budget_aggregate.py` (see "Scripts" above) for the per-category `FY26` totals, filtered to `Status = Active`. **Don't reach for `get_sheet_aggregates` here** — it can't return a per-category dollar sum (see "Scripts" above), so trying it first just costs a round trip before falling back to this same full-sheet pull anyway.
