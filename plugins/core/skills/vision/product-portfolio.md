# Vision — Product Portfolio

Learn it by category, not by SKU. Four categories cover the Vision line — knowing which category a product sits in, and its one-line pitch, matters more than memorizing every SKU inside it. Unlike Aesthetics, these categories map to distinct anatomical segments (anterior vs. posterior eye) and distinct buyer specialties, not just distinct technologies.

| Category | Flagship Products | Technology | The One-Line Pitch |
|---|---|---|---|
| Dry eye | OptiLIGHT, OptiPLUS, Optima IPL, OPT Shields, Antares | IPL (OPT), dual-frequency RF | First & only FDA-cleared IPL for dry eye management |
...
Vision's pitch rests on four sourced "firsts": the first SLT laser, the first argon laser photocoagulator, the first IPL technology for dry eye management, and the first digital dual-path SLT-YAG platform — a 60+ year pattern of being early to each modality it now sells into, not just a long history. That's a different throughline than Aesthetics' "breadth across modalities" claim — Vision doesn't yet have a verified cross-portfolio differentiation claim the way Aesthetics does (see [competitors.md](competitors.md), which is still a placeholder). Until that research happens, lean on the "first" framing per category rather than an unverified breadth claim.
